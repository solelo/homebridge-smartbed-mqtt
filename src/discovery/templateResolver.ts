...
const SIMPLE_VALUE_JSON =
  /^\{\{\s*value_json((?:\.[a-zA-Z_][a-zA-Z0-9_]*|\[['"][^'"]+['"]\])*)\s*(\|\s*(int|float|round)(\((\d+)\))?\s*)?\}\}$/;
 ...
 */
export function parseTemplate(template: string | undefined): ParsedTemplate | undefined {
  if (!template) {
    return { path: [] };
  }
  ...
  if (match) {
    ...
    const filter = match[3] as ParsedTemplate['filter'] | undefined;
