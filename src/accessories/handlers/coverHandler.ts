 ...
 */
export class CoverHandler extends EntityHandler {
  private readonly openPos: number;
  private readonly closedPos: number;
  ...
  setupService(): Service | undefined {
    ...
    const service =
      this.ctx.accessory.getServiceById(S.WindowCovering, subtype) ??
  ...
  private handleSetTargetPosition(homekitPos: number): void {
  ...
  private scaleDeviceToHomekit(devicePos: number): number {
    const span = this.openPos - this.closedPos || 1;
    const pct = ((devicePos - this.closedPos) / span) * 100;
    return clamp(Math.round(pct), 0, 100);
  }

  private scaleHomekitToDevice(homekitPos: number): number {
